...
ext_graphic_seven_data_t tx_client_line;
...
char third2_line[30] = {"FRONT"};
char third3_line[30] = {"SIDE"};
char third4_line[30] = {"SPIN"};
char third5_line[30] = {"SLIP"};
...
static void Draw_line(void)
{
		...
		if((SuperCap_ReceivePacket.cap_v - 12.f)/(23.f-12.f) * 100.f >= 20)
			Figure_Graphic(&tx_client_line.clientData[4],"AL5",update_balance_flag,LINE,2,GREEN,0,0,10,670,20  ,0, 670+(SuperCap_ReceivePacket.cap_v - 12.f)/(23.f-12.f) * 100.f*6,20);
		else if((SuperCap_ReceivePacket.cap_v - 12.f)/(23.f-12.f) * 100.f < 20)
			Figure_Graphic(&tx_client_line.clientData[4],"AL5",update_balance_flag,LINE,2,FUCHSIA,0,0,10,670,20  ,0,  670+(SuperCap_ReceivePacket.cap_v - 12.f)/(23.f-12.f) * 100.f*6,20);
	
    if(fabsf(Control_Info.Measure.chassis_yawangle - Control_Info.Target.chassis_yawangle[1]) < fabsf(Control_Info.Measure.chassis_yawangle - Control_Info.Target.chassis_yawangle[0]))
    {
        Figure_Graphic(&tx_client_line.clientData[2],"AL3",update_balance_flag,LINE,2,YELLOW,0,0,3,1770,(1080*16/20), 0,1770,(1080*14/20));

				if(Control_Info.Target.chassis_yawangle[0] == 268.30f)
				{
					Figure_Graphic(&tx_client_line.clientData[5],"AL6",update_balance_flag,LINE,2,YELLOW,0,0,3,1770,(1080*15/20), 0,1824,(1080*15/20));
				}else if(Control_Info.Target.chassis_yawangle[0] == 88.30f)
				{
					Figure_Graphic(&tx_client_line.clientData[5],"AL6",update_balance_flag,LINE,2,YELLOW,0,0,3,1716,(1080*15/20), 0,1770,(1080*15/20));
				}
    }
    else
    {
        Figure_Graphic(&tx_client_line.clientData[2],"AL3",update_balance_flag,LINE,2,YELLOW,0,0,3,1716,(1080*15/20), 0,1824,(1080*15/20));
				...
				if(Control_Info.Target.chassis_yawangle[0] == 268.30f)
				{
					Figure_Graphic(&tx_client_line.clientData[5],"AL6",update_balance_flag,LINE,2,YELLOW,0,0,3,1770,(1080*15/20), 0,1770,(1080*16/20));
				}else if(Control_Info.Target.chassis_yawangle[0] == 88.30f)
				{
					Figure_Graphic(&tx_client_line.clientData[5],"AL6",update_balance_flag,LINE,2,YELLOW,0,0,3,1770,(1080*15/20), 0,1770,(1080*14/20));
				}
    }
...
void Client_Graph_Init(void)
{
	...
	tx_client_line.txFrameHeader.DataLength = sizeof(ext_student_interactive_header_data_t) + sizeof(graphic_data_struct_t)*7;
