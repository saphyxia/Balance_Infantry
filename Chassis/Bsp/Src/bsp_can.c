...
SuperCap_ReceivePacket_Typedef SuperCap_ReceivePacket={
  .id = 0x026U,
};
...
static void CAN1_RxFifo0RxHandler(uint32_t *StdId,uint8_t data[8])
{
	...
	DJI_Motor_Info_Update(StdId,data,&DJI_Motor[Left_Momentum]);
...
static void CAN2_RxFifo0RxHandler(uint32_t *StdId,uint8_t data[8])
{
  ...
  if(*StdId == SuperCap_ReceivePacket.id)
  {
    SuperCap_ReceivePacket.low_power = (uint8_t)(data[0]);
    SuperCap_ReceivePacket.charge_state = (uint8_t)(data[1]);
    SuperCap_ReceivePacket.chassis_power = ((uint16_t)data[2]<<8 | (uint16_t)data[3]);
    SuperCap_ReceivePacket.ele_quantity = ((uint16_t)data[4]<<8 | (uint16_t)data[5]);
    SuperCap_ReceivePacket.cap_v = ((uint16_t)data[6]<<8 | (uint16_t)data[7]);
  }
