...
void Referee_Frame_Update(uint32_t *StdId, uint8_t *rxBuf)
{
	...
	Referee_Info.robot_status.robot_id = (uint8_t)(rxBuf[0] & 0X80U) >> 7;  // 1 blue 0 red
	Referee_Info.robot_status.robot_level = (uint8_t)(rxBuf[0] & 0X60U) >> 5;
	
	shooter_type = (uint8_t)(rxBuf[0] & 0X0CU) >> 2;
	
	if(Referee_Info.robot_status.robot_level == 1)
	{
		if(shooter_type == 1)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 200;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 15;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 15;
		}
		else if(shooter_type == 2)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 50;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 60;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 15;
		}
		else if(shooter_type == 3)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 75;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 22;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 30;
		}
		else
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 50;
	...
	else if(Referee_Info.robot_status.robot_level == 2)
	{
		if(shooter_type == 1)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 400;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 22;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 15;
		}
		else if(shooter_type == 2)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 100;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 90;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 18;
		}
		else if(shooter_type == 3)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 150;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 37;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 30;
		}
		else
		{
	...
	else if(Referee_Info.robot_status.robot_level == 3)
	{
		if(shooter_type == 1)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 600;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 45;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 15;
		}
		else if(shooter_type == 2)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 150;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 120;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 18;
		}
		else if(shooter_type == 3)
		{
			Referee_Info.robot_status.shooter_id1_17mm_cooling_limit = 200;
			Referee_Info.robot_status.shooter_id1_17mm_cooling_rate = 52;
			Referee_Info.robot_status.shooter_id1_17mm_speed_limit = 30;
		}
		else
		{
	...
	Referee_Info.power_heat.shooter_id1_17mm_cooling_heat = (uint16_t)(rxBuf[1] << 8) | (uint16_t)(rxBuf[2]);
	Referee_Info.shoot_data.bullet_speed = (float)((uint16_t)(rxBuf[3] << 8) | (uint16_t)(rxBuf[4]))/100.f;
}
...
void Referee_Info_Update(uint8_t *Buff,Referee_Info_TypeDef *referee)
{
	do
	{
		...
		referee->index += (uint16_t)(Buff[referee->index + 2] << 8 | Buff[referee->index + 1]) + CRC16_Length + CMDID_Length + FrameHeader_Length;
	}while(Buff[referee->index]==0xA5);
}
